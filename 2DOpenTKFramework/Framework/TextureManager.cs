...
namespace OpenTKFramework.Framework {
    public class TextureManager {
        ...
        private List<TextureInstance> managedTextures;
        ...
        public void Initialize(GameWindow window) {
            if ( this.isInitialized ) Error( "Trying to double intialize texture manager!" );
            this.managedTextures = new List<TextureInstance>();
            ...
            this.managedTextures.Capacity = 100;
        ...
        public void Shutdown() {
            if ( !this.isInitialized ) Error( "Trying to shut down a non initialized texture manager!" );
            // Loop trough all loaded textures
            for ( var i = 0; i < this.managedTextures.Count; ++i ) {
                ...
                if ( this.managedTextures[i].refCount != 0 ) Warning( "Texture reference is > 0: " + this.managedTextures[i].path );
                // Delete the texture from graphics memory
                GL.DeleteTexture( this.managedTextures[i].glHandle );
                // This isn't needed, but let's set the textures element in the array
                // to null, to give GC a hint that it is to be collected.
                this.managedTextures[i] = null;
            }
        ...
        private int LoadGLTexture(string filename, out int width, out int height, bool nearest) {
            ...
            var bmp_data = bmp.LockBits( new Rectangle( 0, 0, bmp.Width, bmp.Height ), ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb );

            // If the texture is non POT, or bigger than 2048 warn the user so they can fix
            // the texutre during development
            if ( !IsPowerOfTwo( bmp.Width ) ) Warning( "Texture width non power of two: " + filename );

            if ( !IsPowerOfTwo( bmp.Height ) ) Warning( "Texture height  non power of two: " + filename );

            if ( bmp.Width > 2048 ) Warning( "Texture width > 2048: " + filename );

            if ( bmp.Height > 2048 ) Warning( "Texture height > 2048: " + filename );
        ...
        public int LoadTexture(string texturePath) {
            if ( !this.isInitialized ) Error( "Trying to load texture without intializing texture manager!" );

            // First, check if the texture is already being managed. If it is, increase ref count and
            // return the textures index.
            for ( var i = 0; i < this.managedTextures.Count; ++i )
                if ( this.managedTextures[i].path == texturePath ) {
                    this.managedTextures[i].refCount += 1;
                    return i;
                }
            ...
            for ( var i = 0; i < this.managedTextures.Count; ++i )
                if ( this.managedTextures[i].refCount <= 0 ) {
                    GL.DeleteTexture( this.managedTextures[i].glHandle );
                    this.managedTextures[i].glHandle = LoadGLTexture( texturePath, out this.managedTextures[i].width, out this.managedTextures[i].height, this.UseNearestFiltering );
                    this.managedTextures[i].refCount = 1;
                    this.managedTextures[i].path     = texturePath;
                    return i;
                }
            ...
            var newTexture = new TextureInstance();
            newTexture.refCount = 1;
            newTexture.glHandle = LoadGLTexture( texturePath, out newTexture.width, out newTexture.height, this.UseNearestFiltering );
            newTexture.path     = texturePath;
            this.managedTextures.Add( newTexture );
            return this.managedTextures.Count - 1;
        }
        ...
        public void UnloadTexture(int textureId) {
            if ( !this.isInitialized ) Error( "Trying to unload texture without intializing texture manager!" );
            this.managedTextures[textureId].refCount -= 1;

            // If we go below -1, no problem. But the system is intended to go only to 0, so lets
            // warn the user that they cleaned up wrong
            if ( this.managedTextures[textureId].refCount < 0 ) Error( "Ref count of texture is less than 0: " + this.managedTextures[textureId].path );
        }

        // Simple getter to access the width of a texture
        public int GetTextureWidth(int textureId) {
            if ( !this.isInitialized ) Error( "Trying to access texture width without intializing texture manager!" );
            return this.managedTextures[textureId].width;
        }

        // Simple getter to access the height of a texture
        public int GetTextureHeight(int textureId) {
            if ( !this.isInitialized ) Error( "Trying to access texture height without intializing texture manager!" );
            return this.managedTextures[textureId].height;
        }

        // Simple getter to access the size of a texture
        public Size GetTextureSize(int textureId) {
            if ( !this.isInitialized ) Error( "Trying to access texture size without intializing texture manager!" );
            return new Size( this.managedTextures[textureId].width, this.managedTextures[textureId].height );
        }
        ...
        public void Draw(int textureId, Point screenPosition) {
            if ( !this.isInitialized ) Error( "Trying to draw texture without intializing texture manager!" );
            ...
            var left   = 0.0f;
            var top    = 0.0f;
            var right  = left + this.managedTextures[textureId].width;
            var bottom = top  + this.managedTextures[textureId].height;
            ...
            GL.BindTexture( TextureTarget.Texture2D, this.managedTextures[textureId].glHandle );
        ...
        public void Draw(int textureId, Point screenPosition, float scale, Rectangle sourceSection) { Draw( textureId, screenPosition, new PointF( scale, scale ), sourceSection ); }
        ...
        public void Draw(int textureId, Point screenPosition, PointF scale, Rectangle sourceSection) {
            ...
            float left   = 0;
            float top    = 0;
            var   right  = left + sourceSection.Width;
            var   bottom = top  + sourceSection.Height;

            var wRecip = 1.0f / this.managedTextures[textureId].width;
            var hRecip = 1.0f / this.managedTextures[textureId].height;

            var uvLeft   = sourceSection.X * wRecip;
            var uvTop    = sourceSection.Y * hRecip;
            var uvRight  = uvLeft + sourceSection.Width  * wRecip;
            var uvBottom = uvTop  + sourceSection.Height * hRecip;
        ...
        public void Draw(int textureId, Point screenPosition, float scale, Rectangle sourceSection, float rotation) {
            var rotationCenter = new Point( sourceSection.Width / 2, sourceSection.Height / 2 );
        ...
        public void Draw(int textureId, Point screenPosition, float scale, Rectangle sourceSection, Point rotationCenter, float rotation = 0.0f) { Draw( textureId, screenPosition, new PointF( scale, scale ), sourceSection, rotationCenter, rotation ); }
        ...
        public void Draw(int textureId, Point screenPosition, PointF scale, Rectangle sourceSection, Point rotationCenter, float rotation = 0.0f) {
            ...
            GL.Translate( rotationCenter.X * scale.X, rotationCenter.Y * scale.Y, 0.0f );
